...
import { getGCFUrl } from '../config/gcf';
...
/**
 * Create a GCF client with the configured URL
 */
export function createGCFClient(): CreditReportProcessorClient {
  return new CreditReportProcessorClient(getGCFUrl());
}

/**
 * React Hook for credit report processing
 */
export function useCreditReportProcessor(supabaseClient: any) {
  const gcfUrl = getGCFUrl();
